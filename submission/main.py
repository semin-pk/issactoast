...
def resolve_input_path(base_dir: Path, path_value: str) -> Path:
    input_path = resolve_path(base_dir, path_value)
    if input_path.exists():
        return input_path

    for candidate in (
        base_dir / "box_sequence",
        base_dir / "generated_sequences" / "holdout",
        base_dir / "generated_sequences",
    ):
        if candidate.exists():
            print(f"[WARN] input_path not found: {input_path}. using {candidate}")
            return candidate
...
def main() -> None:
    ...
    input_path = resolve_input_path(here, cfg["input_path"])
    ...
    visualize_enabled = bool(cfg.get("visualize", {}).get("enabled", False))
    visualize_fn = None
    if visualize_enabled:
        from visualize import visualize as visualize_fn
    ...
    for idx, input_file in enumerate(input_files):
        ...
        vis_path = None
        if visualize_enabled and visualize_fn is not None:
